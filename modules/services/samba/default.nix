{
  ...
  cfg = config.x-next.services.samba;
  ...
  shares-submodule = with types;
    submodule ({name, ...}: {
      options = {
        ...
        only-owner-editable = mkBoolOpt false "Whether the share is only writable by the system owner (x-next.user.name).";

        extra-config = mkOpt attrs {} "Extra configuration options for the share.";
      };
    });
in {
  options.x-next.services.samba = with types; {
