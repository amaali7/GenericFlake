{
  ...
  cfg = config.x-next.services.writefreely;
  ...
  withSqlite = text:
    ...
    '';
in {
  options.x-next.services.writefreely = {
    enable =
    ...
    database = {
      ...
      createLocally = mkOption {
        ...
        default = false;
        description = ''
          When <option>x-next.services.writefreely.database.type</option> is set to
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = cfg.host != "";
        message = "x-next.services.writefreely.host must be set";
      }
      {
        assertion = isMysqlLocal -> cfg.database.passwordFile != null;
        message = "x-next.services.writefreely.database.passwordFile must be set if x-next.services.writefreely.database.createLocally is set to true";
      }
      {
        assertion = isSqlite -> !cfg.database.createLocally;
        message = "x-next.services.writefreely.database.createLocally has no use when x-next.services.writefreely.database.type is set to sqlite3";
      }
    ];
