...
with lib.internal; let
  cfg = config.x-next.services.openssh;
  ...
  other-hosts =
    lib.filterAttrs
    (key: host:
      key != name && (host.config.x-next.user.name or null) != null)
  ...
  other-hosts-config =
    ...
    (
      name: let
        ...
        remote-user-name = remote.config.x-next.user.name;
...
in {
  options.x-next.services.openssh = with types; {
  ...
  config = mkIf cfg.enable {
    ...
    x-next.user.extraOptions.openssh.authorizedKeys.keys =
      cfg.authorizedKeys;
