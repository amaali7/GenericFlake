...
with lib.internal; let
  cfg = config.x-next.desktop.gnome;
...
in {
  options.x-next.desktop.gnome = with types; {
    enable =
      mkBoolOpt false "Whether or not to use Gnome as the desktop environment.";
    wallpaper = {
      light = mkOpt (oneOf [str package]) pkgs.x-next.wallpapers.nord-rainbow-light-nix "The light wallpaper to use.";
      dark = mkOpt (oneOf [str package]) pkgs.x-next.wallpapers.nord-rainbow-dark-nix "The dark wallpaper to use.";
    };
  ...
  config = mkIf cfg.enable {
    x-next.system.xkb.enable = true;
    x-next.desktop.addons = {
      gtk = enabled;
      wallpapers = enabled;
    ...
    environment.systemPackages = with pkgs;
      [
        (hiPrio x-next.xdg-open-with-portal)
        wl-clipboard
        gnome.gnome-tweaks
    ...
    systemd.services.x-next-user-icon = {
      ...
      script = ''
        config_file=/var/lib/AccountsService/users/${config.x-next.user.name}
        icon_file=/run/current-system/sw/share/x-next-icons/user/${config.x-next.user.name}/${config.x-next.user.icon.fileName}
    ...
    x-next.home.extraOptions = {
      dconf.settings = let
        user = config.users.users.${config.x-next.user.name};
        ...
        nested-default-attrs {
          "org/gnome/shell" = {
            ...
            favorite-apps =
              ...
              ++ optional config.x-next.apps.firefox.enable "firefox.desktop"
              ++ optional config.x-next.apps.vscode.enable "code.desktop"
              ++ optional config.x-next.desktop.addons.foot.enable "foot.desktop"
              ++ optional config.x-next.apps.logseq.enable "logseq.desktop"
              ++ optional config.x-next.apps.discord.enable "discord.desktop"
              ++ optional config.x-next.apps.element.enable "element-desktop.desktop"
              ++ optional config.x-next.apps.steam.enable "steam.desktop";
          };
          ...
          "org/gnome/shell/extensions/Logo-menu" = {
            ...
            menu-button-terminal =
              if config.x-next.desktop.addons.term.enable
              then lib.getExe config.x-next.desktop.addons.term.pkg
